...
const KEY_INFOS: [KeyInfo; 26] = [KeyInfo {
...
fn check_whisky(_info: &KeyInfo, key: &Key) {
    ...
    assert_eq!(sub1.expiration_time(),
               Some(UNIX_EPOCH + Duration::from_secs(1129636886)));
    assert_eq!(sub2.expiration_time(),
               Some(UNIX_EPOCH + Duration::from_secs(1129636939)));
}
...
fn test_keylist() {
    ...
    for key in keys.by_ref() {
        ...
        assert!(!primary.is_card_key());
        assert_eq!(primary.card_serial_number(), Err(None));
        assert_eq!(primary.algorithm(), gpgme::KeyAlgorithm::Dsa);
        ...
        assert!(!secondary.is_card_key());
        assert_eq!(secondary.card_serial_number(), Err(None));
        assert_eq!(secondary.algorithm(), gpgme::KeyAlgorithm::ElgamalEncrypt);
        ...
        assert_eq!(key.user_ids().count(),
                   info.uid.iter().filter(|u| u.is_some()).count());
        for (actual, expected) in key.user_ids().zip(info.uid.iter().filter_map(|u| u.as_ref())) {
            ...
            assert_eq!(actual.validity(), gpgme::Validity::Unknown);
    ...
    assert!(!fail_if_err!(keys.finish()).is_truncated());
