...
fn test_create_key() {
    common::with_test_harness(|| {
        ...
        assert!(!uid.is_bad());
        assert_matches!(uid.name(), Ok("test user"));
        assert_matches!(uid.email(), Ok("test@example.com"));
        assert_matches!(uid.id(), Ok("test user <test@example.com>"));

        let res = ctx
            .create_subkey_with_flags(&key, "future-default", expiration, CreateKeyFlags::AUTH)
            .unwrap();
        let creation = SystemTime::now();
        ...
        let sub = key
            .subkeys()
            .find(|k| k.fingerprint_raw() == res.fingerprint_raw())
            .unwrap();
        assert!(!sub.is_bad());
        ...
        assert!(sub.can_encrypt());
        assert!(!sub.can_authenticate());
        assert_matches!(sub.expiration_time(), None);
    })
}
