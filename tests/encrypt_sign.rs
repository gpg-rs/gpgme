...
fn test_encrypt_sign() {
    ...
    ctx.with_passphrase_provider(passphrase_cb, |mut ctx| {
        ...
        let result = fail_if_err!(ctx.encrypt_and_sign(&[key1, key2], ops::ENCRYPT_ALWAYS_TRUST,
        ...
        if gpgme::init().check_version("1.4.3") {
            ...
            check_result(fail_if_err!(ctx.encrypt_and_sign(None, ops::EncryptFlags::empty(),
                &mut input, &mut output)).1, ops::SIGN_MODE_NORMAL);
        }
    });
}
